...
public class List extends HttpServlet {
	...
	XmlIO xio = new XmlIO();
	xio.loadItemsFromFile(shop, items);
	String uri = request.getRequestURI();
	XmlItem xi = new XmlItem();
	xi.loadItemsFromFile(basket, item);
	String name = " ";
	...
	for(int i = 0; i < items.size(); i++){
		if( uri.equals("/shop/List/add" + items.get(i).getArt()) ) {
			if(session != null){
				...
				item.add(bi);
				xi.saveItemsToFile(basket, item);
			}
	...
	new XmlItem().loadItemsFromFile(basket, item);
	...
	public String getMainPage(HttpServletRequest request, HttpServletResponse response){
		HttpSession session = request.getSession(false);
		items = new ArrayList<Item>();String shop = "";
		try{
			Scanner in = new Scanner(new File("shop.txt"));
			while(in.hasNext())
				shop += in.nextLine();
